...
- **Backend**: Express.js, WebSocket Server (ws), OpenAI integration via local server, PostgreSQL (localhost) with Drizzle ORM.
- **Trading Logic**:
...
- **Database**: PostgreSQL (localhost) with Drizzle ORM for persistent data, in-memory storage for real-time data.
...
- **AI Integration**: OpenAI (via local server)
